...
export async function GET(request: NextRequest) {
  try {
    ...
    const groupedResults = results.reduce((acc, message) => {
      ...
      return acc;
    }, {} as Record<string, { conversationId: string; conversationTitle: string; conversationCreatedAt: Date; messages: Array<{ id: string; role: string; content: string; createdAt: Date }> }>);
